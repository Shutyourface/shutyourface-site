    ...
    <main id="top" className="min-h-screen bg-white text-black">
      ...
      <div className="mx-auto max-w-[1500px] px-3 py-4">
        ...
        {moreFaceStories.length ? (
          <section className="mt-5 grid grid-cols-4 items-start gap-x-5 gap-y-6">
            {moreFaceStories.map((story, index) => (
              <a key={story._id || story.headline} href={story.url || "#"} {...externalLinkProps(story.url)} className={`${story.featured ? "text-3xl" : index % 4 === 0 ? "text-2xl" : "text-lg"} block border-b-2 border-black pb-3 font-black uppercase leading-none underline decoration-2 hover:bg-red-700 hover:text-white`}>
                {story.imageUrl && !story.imageHidden ? (
                  <img src={story.imageUrl} alt="" className="mb-2 h-32 w-full border-2 border-black object-cover object-top grayscale contrast-125" loading="lazy" />
                ) : null}
                {story.headline}
              </a>
            ))}
